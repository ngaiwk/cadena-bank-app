...
function App() {
  ...
  const [bankTotalBalance, setBankTotalBalance] = useState(null);
  const [currentBankName, setCurrentBankName] = useState(null);
  const [customerAddress, setCustomerAddress] = useState(null);
  const [error, setError] = useState(null);
  const contractAddress = '0xf3082dfebf29E871A3d791a618137232c3012aea';
  ...
  const getbankBalanceHandler = async () => {
    try {
      if (window.ethereum) {
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        const signer = provider.getSigner();
        ...
        if (owner.toLowerCase() === account.toLowerCase()) {
          ...
          setBankTotalBalance(utils.formatEther(balance));
          console.log("Retrieved bank balance...", balance);
        }
  ...
  const withDrawMoneyHandler = async (event) => {
    try {
      event.preventDefault();
      if (window.ethereum && inputValue.withdraw > 0) {
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        const signer = provider.getSigner();
        const bankContract = new ethers.Contract(contractAddress, contractABI, signer);
        ...
        if (inputValue.withdraw <= 0) {
          console.log("Try to input 0 or negative value");
  ...
  return (
    <main className="main-container">
      ...
      <section className="customer-section px-10 pt-5 pb-10">
        ...
        <div className="mt-7 mb-9">
          <form className="form-style">
            <input
              ...
              name="deposit"
              placeholder="0.0000 RinkebyETH"
              value={inputValue.deposit}
            />
            <button
              className="btn-purple"
              onClick={deposityMoneyHandler}>Deposit Money In RinkebyETH</button>
          </form>
        </div>
        ...
        <div className="mt-5">
          <p><span className="font-bold">Bank Total Balance: </span>{bankTotalBalance}</p>
        </div>
        <div className="mt-5">
